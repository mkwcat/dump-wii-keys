...
using IOVector = ioctlv;

static u8 s_tmpBuffer[SDCard::TMP_BUFFER_SIZE] alignas(0x20);

static SDCard s_slot0;
...
SDCard::SDCard()
{
    ...
    if (m_fd < 0) {
        std::printf("Failed to open /dev/sdio/slot0: Returned error %i", m_fd);
        return;
    }

    m_tmpBuffer = s_tmpBuffer;
...
bool SDCard::ResetCard()
{
    ...
    if (IOS_Ioctl(m_fd, IOCTL_RESET_CARD, nullptr, 0, &out, sizeof(out)) < 0) {
        std::printf("SDCard: Failed to reset interface");
        return false;
    }

    std::printf("SDCard: Successfully reset interface");
    m_rca = out >> 16;
    return true;
}
...
bool SDCard::GetStatus(u32* status)
{
    ...
    if (IOS_Ioctl(m_fd, IOCTL_GET_STATUS, nullptr, 0, &out, sizeof(out)) < 0) {
        std::printf("SDCard: Failed to get status");
        return false;
    }
...
bool SDCard::ReadHCR(u8 reg, u8 size, u32* val)
{
    ...
    if (IOS_Ioctl(
            m_fd, IOCTL_READ_HCR, &regOp, sizeof(regOp), &out, sizeof(out)
        ) < 0) {
        std::printf(
            "SDCard: Failed to read host controller register 0x%X", reg
        );
        return false;
    }
...
bool SDCard::WriteHCR(u8 reg, u8 size, u32 val)
{
    ...
    if (IOS_Ioctl(m_fd, IOCTL_WRITE_HCR, &regOp, sizeof(regOp), nullptr, 0) <
        0) {
        std::printf(
            "SDCard: Failed to write to host controller register 0x%X", reg
        );
        return false;
    }
...
bool SDCard::SetClock(u32 clock)
{
    ...
    if (IOS_Ioctl(m_fd, IOCTL_SET_CLOCK, &in, sizeof(in), nullptr, 0) < 0) {
        std::printf("SDCard: Failed to set clock");
        return false;
    }
    ...
    if (buffer || m_isSdhc) {
        ...
        if (IOS_Ioctlv(m_fd, IOCTLV_SEND_COMMAND, 2, 1, vec) < 0) {
            if (command != CMD_SELECT) {
                std::printf("SDCard: Failed to send command 0x%X", command);
            }
            return false;
        }
    } else {
        if (IOS_Ioctl(
                m_fd, IOCTL_SEND_COMMAND, &request, sizeof(request), &out,
                sizeof(u32) * 4
            ) < 0) {
            if (command != CMD_SELECT) {
                std::printf("SDCard: Failed to send command 0x%X", command);
...
{
    return SendCommand(
        CMD_SET_BLOCKLEN, 3, RESPONSE_TYPE_R1, blockLength, 0, 0, nullptr,
        nullptr
    );
}
...
{
    if (!SendCommand(
            CMD_APP_CMD, 3, RESPONSE_TYPE_R1, m_rca << 16, 0, 0, nullptr,
            nullptr
        )) {
        return false;
    }
...
)
{
    ...
    return SendCommand(
        command, 3, RESPONSE_TYPE_R1, firstBlock, sectorCount, SECTOR_SIZE,
        buffer, nullptr
    );
}
...
)
{
    ...
    u8* buffer8 = static_cast<u8*>(buffer);

    while (sectorCount > 0) {
        u32 chunkSectorCount = std::min<u32>(sectorCount, TMP_SECTOR_COUNT);
        if (isWrite) {
            memcpy(m_tmpBuffer, buffer8, chunkSectorCount * SECTOR_SIZE);
        }
        if (!TransferAligned(
        ...
        if (!isWrite) {
            memcpy(buffer8, m_tmpBuffer, chunkSectorCount * SECTOR_SIZE);
        }
        firstSector += chunkSectorCount;
        sectorCount -= chunkSectorCount;
        buffer8 += chunkSectorCount * SECTOR_SIZE;
    }
 ...
 */
bool SDCard::WaitForInsert(bool state)
{
    return SendCommand(
        CMD_WAIT_EVENT, 0, 0, state ? 1 : 2, 0, 0, nullptr, nullptr
    );
}
 ...
 */
void SDCard::CancelWaitForInsert(bool state)
{
    SendCommand(
        CMD_CANCEL_WAIT_EVENT, 0, 0, state ? 1 : 2, 0, 0, nullptr, nullptr
    );
}
...
bool SDCard::Init()
{
    ...
    if (!(status & STATUS_CARD_INSERTED)) {
        std::printf("SDCard: No card inserted");
        return false;
    }

    if (!(status & STATUS_TYPE_MEMORY)) {
        std::printf("SDCard: Not a memory card");
        return false;
    }
    ...
    if (!Enable4BitBus()) {
        std::printf("SDCard: Failed to enable 4-bit bus");
        return false;
    }
