...
void Layout_TextBox::Draw()
{
    ...
    for (u32 i = 0; i < len; i++) {
        if (m_text[i] == L'\n') {
            lineWidth[currentLine++] = currentLineWidth;
            if (currentLine >= int(std::size(lineWidth))) {
                currentLine = std::size(lineWidth) - 1;
            }
    ...
    float startXPerLine[64] = {};

    for (int i = 0; i < currentLine; i++) {
        startXPerLine[i] = startX;

        switch (m_textAnchor) {
        case Anchor::TOP_LEFT:
        case Anchor::MIDDLE_LEFT:
        case Anchor::BOTTOM_LEFT:
            startXPerLine[i] -= m_width / 2;
            break;

        case Anchor::TOP_CENTER:
        case Anchor::MIDDLE_CENTER:
        case Anchor::BOTTOM_CENTER:
            startXPerLine[i] -= lineWidth[i] / 2;
            break;

        case Anchor::TOP_RIGHT:
        case Anchor::MIDDLE_RIGHT:
        case Anchor::BOTTOM_RIGHT:
            startXPerLine[i] += m_width / 2 - lineWidth[i] * squishX;
            break;
        }
    }
    ...
    // Draw text
    float x = startXPerLine[0], y = startY - posHeight;
    currentLine = 0;
    for (u32 i = 0; i < len; i++) {
        if (m_text[i] == L'\n') {
            x = startXPerLine[++currentLine];
