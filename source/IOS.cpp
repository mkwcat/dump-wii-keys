...
/**
 * Returns true if the program is running on Wii U hardware.
 */
bool IOS::IsCafe()
{
    volatile u32* const HW_CHIPREVID =
        reinterpret_cast<volatile u32*>(0xCD800214);
    volatile u32* const HW_CHIPREVID_MIRR =
        reinterpret_cast<volatile u32*>(0xCD800A14);
    volatile u32* const LT_CHIPREVID =
        reinterpret_cast<volatile u32*>(0xCD8005A0);

    if (*HW_CHIPREVID == 0 || *HW_CHIPREVID == *HW_CHIPREVID_MIRR) {
        return false;
    }

    if ((*LT_CHIPREVID >> 16) != 0xCAFE) {
        return false;
    }
